...
**Modern, fast, and modular Jamstack-based website template.** Kickstart your project in seconds with the power of Astro 5, Svelte 5, and Tailwind CSS 4.
...
Multi-lingual static site template built with Astro 5, Svelte 5, and Tailwind CSS 4. Includes Blog (tags/categories), product catalog, in-site search (Cmd+K), dark/light theme, dynamic slider, and PWA support.

Created by: [Bahadır Doğru](https://bahadirdogru.com)

Content and SEO are generated by Astro; interactive components are written in Svelte and hydrated on relevant pages. Build output (`dist/`) can be deployed to any static hosting (including GitHub Pages).
...
2. `astro build` — builds the site into `dist/`
...
Tests the `dist/` output on a local server.
...
Then upload the `dist/` folder to your hosting (for GitHub Pages, you can push the `dist` content to the repo or `gh-pages` branch; Actions for automatic build can also be used).
