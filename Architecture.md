...
│  ├── components/                                                       │
│  │   ├── blocks/               (Modular UI Blocks: Hero, Feature vb.) │
│  │   ├── DarkModeToggle.svelte                                        │
│  │   ├── Slider.svelte                                                │
│  │   ├── SearchFuse.svelte                                            │
│  │   ├── ProductCarousel.svelte                                       │
│  │   ├── CompareBar.svelte, FavoritesBadge.svelte                     │
│  │   ├── BackToTop.svelte, ScrollProgress.svelte                      │
│  │   ├── ErrorBoundary.svelte, Toast.svelte                           │
│  │   └── ...                                                          │
│  ├── content/                                                         │
│  ├── content.config.js          (Content Layer API: posts + pages)     │
│  │   ├── posts/*.md            (blog yazıları)                        │
│  │   ├── pages/*.md            (about, getting-started, showcase)     │
...
│  ├── lib/                      search-index.js → public/search-index  │
│  ├── pages/                    ([lang] prefix for all, incl. TR)      │
│  ├── config/site.js            (tekil config: site, features, i18n)   │
...
│                         BUILD PIPELINE                                 │
│                                                                        │
│  1. node src/lib/search-index.js → public/search-index.json           │
│  2. astro build                  → docs/ (sitemap/PWA features'a göre)│
└─────────────────────────────────────────────────────────────────────────┘
                                   │
                              Deploy (static host)
                                   │
...
│                                                                        │
│  docs/ statik olarak sunulur. Sitemap, RSS, PWA Astro/Vite ile.       │
└─────────────────────────────────────────────────────────────────────────┘
```
...
- **astro.config.mjs**: `site` → site.js'deki site.url; `outDir` → `"docs"`; sitemap ve VitePWA yalnızca features.sitemap / features.pwa true ise eklenir.
...
src/pages/**/*.astro
    ...
    └── Content Layer API (posts, pages) → getCollection, getEntry, render
            │
...
- **CSS**: `src/styles/global.css` → `@custom-variant dark` + `@theme` ile primary (blue-600), secondary (emerald-500), accent (amber-500); yüzey ve metin renkleri slate paleti.
...
- **Sayfalar**: `src/pages/[lang]/products/[slug].astro` → getStaticPaths + getProductBySlug ile HTML üretimi.
...
- **Bloklar**: `src/components/blocks/` altında HeroSection, FeatureSection, CTASection, StatsSection, TestimonialsSection ve FAQSection bileşenleri bulunur. `BlockRenderer.svelte` frontmatter'daki `blocks` dizisinden hangisinin render edileceğini seçer.
- **Etkileşim bileşenleri**: BackToTop, CompareBar, CompareButton, FavoriteButton, FavoritesBadge, FavoritesList, ReadingList, ReadingListButton, ShareButton, KeyboardShortcuts, QuickView, RecentlyViewed, ScrollProgress, Toast, ErrorBoundary.
- **Blog bileşenleri**: BlogListLoadMore, BlogListWithSkeleton, BlogSidebar (Astro), CopyCode, TableOfContents, RecentPosts (Astro).
- **Ürün bileşenleri**: ProductCarousel, ProductFilter, ProductGridWithSkeleton, ProductsFilterSort.
- **Arama & Tema**: SearchFuse, Search, DarkModeToggle, ThemeToggle.
- **Diğer**: Slider, LazyImage, Lightbox, LoadMore, SkeletonCard, SkeletonListItem, Stories, Counter, PwaUpdateBanner.
...
| `docs/` | — | astro build | Deploy edilen çıktı |
